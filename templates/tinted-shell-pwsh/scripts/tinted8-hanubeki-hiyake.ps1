# tinted-shell (tinted8) hanubeki-hiyake for PowerShell
# scheme made by Tinted Theming (https://github.com/tinted-theming)

$Env:TINTED8_THEME = "hanubeki-hiyake"

Write-Host -NoNewline "`e]4;0;rgb:29/31/3c`e\"
Write-Host -NoNewline "`e]4;1;rgb:9e/53/6d`e\"
Write-Host -NoNewline "`e]4;2;rgb:48/84/50`e\"
Write-Host -NoNewline "`e]4;3;rgb:bc/87/34`e\"
Write-Host -NoNewline "`e]4;4;rgb:3c/6d/a2`e\"
Write-Host -NoNewline "`e]4;5;rgb:73/5f/a2`e\"
Write-Host -NoNewline "`e]4;6;rgb:36/9e/ad`e\"
Write-Host -NoNewline "`e]4;7;rgb:d8/c3/ab`e\"

Write-Host -NoNewline "`e]4;8;rgb:43/47/4e`e\"
Write-Host -NoNewline "`e]4;9;rgb:bc/6b/86`e\"
Write-Host -NoNewline "`e]4;10;rgb:61/9e/68`e\"
Write-Host -NoNewline "`e]4;11;rgb:d9/a1/4c`e\"
Write-Host -NoNewline "`e]4;12;rgb:57/87/be`e\"
Write-Host -NoNewline "`e]4;13;rgb:8d/79/be`e\"
Write-Host -NoNewline "`e]4;14;rgb:56/b9/c8`e\"
Write-Host -NoNewline "`e]4;15;rgb:ff/ee/dc`e\"

Write-Host -NoNewline "`e]10;rgb:29/31/3c`e\"

if ($Env:TINTED8_SHELL_SET_BACKGROUND -ne "false") {
    Write-Host -NoNewline "`e]11;rgb:d8/c3/ab`e\"
}

# Write-Host -NoNewline "`e]12;rgb:d8/c3/ab`e\"
Write-Host -NoNewline "`e]12;rgb:29/31/3c`e\"

if ($(Test-Path "Env:TINTED_SHELL_ENABLE_TINTED8_VARS")) {
    $Env:TINTED8_COLOR_BLACK_NORMAL_HEX = "29313c"
    $Env:TINTED8_COLOR_RED_NORMAL_HEX = "9e536d"
    $Env:TINTED8_COLOR_GREEN_NORMAL_HEX = "488450"
    $Env:TINTED8_COLOR_YELLOW_NORMAL_HEX = "bc8734"
    $Env:TINTED8_COLOR_BLUE_NORMAL_HEX = "3c6da2"
    $Env:TINTED8_COLOR_MAGENTA_NORMAL_HEX = "735fa2"
    $Env:TINTED8_COLOR_CYAN_NORMAL_HEX = "369ead"
    $Env:TINTED8_COLOR_WHITE_NORMAL_HEX = "d8c3ab"

    $Env:TINTED8_COLOR_BLACK_BRIGHT_HEX = "43474e"
    $Env:TINTED8_COLOR_RED_BRIGHT_HEX = "bc6b86"
    $Env:TINTED8_COLOR_GREEN_BRIGHT_HEX = "619e68"
    $Env:TINTED8_COLOR_YELLOW_BRIGHT_HEX = "d9a14c"
    $Env:TINTED8_COLOR_BLUE_BRIGHT_HEX = "5787be"
    $Env:TINTED8_COLOR_MAGENTA_BRIGHT_HEX = "8d79be"
    $Env:TINTED8_COLOR_CYAN_BRIGHT_HEX = "56b9c8"
    $Env:TINTED8_COLOR_WHITE_BRIGHT_HEX = "ffeedc"

    $Env:TINTED8_COLOR_BLACK_DIM_HEX = "1e2631"
    $Env:TINTED8_COLOR_RED_DIM_HEX = "823b54"
    $Env:TINTED8_COLOR_GREEN_DIM_HEX = "2f6a39"
    $Env:TINTED8_COLOR_YELLOW_DIM_HEX = "9e6d1c"
    $Env:TINTED8_COLOR_BLUE_DIM_HEX = "1f5588"
    $Env:TINTED8_COLOR_MAGENTA_DIM_HEX = "684380"
    $Env:TINTED8_COLOR_CYAN_DIM_HEX = "008392"
    $Env:TINTED8_COLOR_WHITE_DIM_HEX = "b7a99a"
}
